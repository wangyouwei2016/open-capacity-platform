...
import com.open.capacity.rule.exception.InValidRuleBeanException;
...
public class EasyRulesTemplate implements InitializingBean {
	 ...
	 */
	private static final Map<String, Rules> RULES_MAP = new ConcurrentHashMap<>();
	...
	/**
	 * Check weather the Group exists or not thrown exception if checking failure
	 * 
	 ...
	 * @param fact
	 */
	public void fire(Rules rules, String factName, Object fact) {
		Objects.requireNonNull(fact, "Fact object must not be NULL.");
		Facts facts = new Facts();
		facts.put(StringUtils.hasText(factName) ? factName : DEFAULT_FACT_NAME, fact);
	...
	public void afterPropertiesSet() throws Exception {
		...
		// validate
		if (ruleBeans.values().stream()
				.anyMatch(ruleBean -> !AnnotationUtils.isCandidateClass(ruleBean.getClass(), Rule.class))) {
		...
		Map<String, List<Object>> ruleBeanGroups = ruleBeans.values().stream()
				.collect(Collectors.groupingBy(ruleBean -> Objects
						.requireNonNull(AnnotationUtils.findAnnotation(ruleBean.getClass(), RuleGroup.class)).name()));

		// store for using after
		ruleBeanGroups.forEach((key, rulesBeans) -> RULES_MAP.put(key, new Rules(rulesBeans.toArray())));
	}
}
